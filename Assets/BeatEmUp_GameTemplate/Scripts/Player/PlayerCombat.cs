...
public class PlayerCombat : MonoBehaviour {
	...
	public DamageObject BeamData; //beam attack
	...
	private List<PLAYERSTATE> AttackStates = new List<PLAYERSTATE> { PLAYERSTATE.IDLE, PLAYERSTATE.MOVING, PLAYERSTATE.JUMPING, PLAYERSTATE.PUNCH, PLAYERSTATE.KICK, PLAYERSTATE.DEFENDING, PLAYERSTATE.BEAM }; //a list of states where the player can attack
	...
	private float getAttackRange() {
		if (playerState.currentState == PLAYERSTATE.PUNCH && attackNum <= PunchAttackData.Length) {
			...
			Debug.Log("BEAM RANGE IS " + BeamData.range);
			return BeamData.range;
		} else {
			return 0f;
		}
	}
